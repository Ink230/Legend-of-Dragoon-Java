...
final class Track {
  ...
  void encode() {
    ...
    try(final MemoryStack memoryStack = stackPush()) {
      ...
      final long encoder = OpusEnc.ope_encoder_create_file(this.path, comments, 37800, channels, 0, error);
  ...
  void addAudioFrame(final FileData sector) {
    this.isStereo = (sector.readUByte(19) & 0x01) == 1;
  ...
  private ShortList decodeNibbles(final FileData xaApdcm, final int position, final int blk, final int nibble, final int lr) {
    final ShortList list = new ShortArrayList();

    final int shift = 12 - (xaApdcm.readUByte(position + 4 + blk * 2 + nibble) & 0x0f);
    final int filter = (xaApdcm.readUByte(position + 4 + blk * 2 + nibble) & 0x30) >> 4;

    final int f0 = positiveXaAdpcmTable[filter];
    final int f1 = negativeXaAdpcmTable[filter];

    for(int i = 0; i < 28; i++) {
      final int t = signed4bit((byte)(xaApdcm.readUByte(position + 16 + blk + i * 4) >> nibble * 4 & 0x0f));
